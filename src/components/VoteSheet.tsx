...
export function VoteSheet({
  ...
  return (
    <dialog
      ...
      className="rounded-2xl border-0 bg-paper p-0 text-ink shadow-[0_24px_60px_-24px_rgba(20,18,15,0.45)] backdrop:bg-black/40 backdrop:backdrop-blur-sm"
      style={{
    ...
    >
      <form
        action={action}
        className="flex max-h-[inherit] flex-col gap-4 overflow-y-auto p-5 sm:gap-5 sm:p-7"
        style={{ overscrollBehavior: "contain" }}
      >
        <input type="hidden" name="choice" value={choice ?? ""} />
        <header className="flex items-baseline justify-between gap-3">
          <h2 className="text-lg font-bold tracking-tight sm:text-xl">اخترت {label}</h2>
          <span className="h-3 w-3 rounded-full" style={{ background: accent }} aria-hidden />
        </header>
        <p className="text-[13px] leading-6 text-ink/70 sm:text-[13.5px]">
        ...
        <label className="flex flex-col gap-2 text-[13px] font-medium">
          ...
          <input
            ...
            className="h-12 rounded-lg border border-black/15 bg-white px-3 text-[16px] outline-none transition-colors focus:border-ink"
          />
        </label>
        <label className="flex flex-col gap-2 text-[13px] font-medium">
        ...
        <div className="mt-1 flex items-center justify-between gap-3">
          <button
            type="button"
            onClick={onClose}
            className="h-11 rounded-full px-4 text-[13px] text-ink/70 hover:bg-black/5"
          >
            إلغاء
          </button>
          <button
            type="submit"
            disabled={pending}
            className="h-11 flex-1 rounded-full px-5 text-[13.5px] font-semibold text-white disabled:opacity-60 sm:flex-none"
